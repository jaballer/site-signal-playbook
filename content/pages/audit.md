...
blocks:
  - type: table
    ...
    rows:
      - - "**Pass**"
        - Meets the standard on every priority page or template
        - One line of evidence. Don't pad the report with passes.
      - - "**Watch**"
        - Partly meets it, or is fine now but at risk
  ...
  - type: collection
    collection: audit
  - type: flow
    heading: From checks to findings
    intro: Nobody buys a checklist. The audit is presented as the ten findings that matter, in order.
    items:
      - title: Group fails into findings
        text: "Several failed checks usually share one cause: a template, a rendering approach, a missing page type. One cause, one finding."
      - title: Attach revenue at stake
        text: "Use the opportunity model: the leads the affected topics or pages could produce, as a range."
